...
We can also join the `abbrev` dataframe to both the `surveys` and the `election_data` dataframes. In that way, that will leave us with just two dataframes: `election_data` and `surveys` which will make our coding easier later.
